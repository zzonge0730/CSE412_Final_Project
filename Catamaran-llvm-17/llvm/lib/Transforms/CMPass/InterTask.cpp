// LLVM 14: InterTask는 현재 비활성화되어 있으므로, 이 파일도 조건부 컴파일
#if 0
#include "InterTask.h"
...
bool InterTask::runOnModule(Module &M) {
    ...
    for (auto *CI : CIToBeSpawned) {
        ...
        uint32_t totalNumArgs = fun->arg_size();
        ...
        for (auto argIt = fun->arg_begin(); argIt != fun->arg_end(); ++argIt, ++wrapperFuncArgIt) {
        ...
        FunctionCallee funCallee(fun);
        CallInst * callInst = CallInst::Create(funCallee, castArgs, "", entryBB);
        ...
        CallInst::Create(ctorCallee, needArgs, "", CI);
        ...
        if (curBB != lastBasicBlock) {
            ...
            CallInst::Create(joinCallee, {id}, "", joinPoint);
        }
...
Constant * InterTask::generateJoin() {
    ...
    FunctionCallee joinCallee = this->mo->getOrInsertFunction("_Z4joinj", joinFuncType);
    return cast<Constant>(joinCallee.getCallee());
}
...
void InterTask::genCtorForSpawn(Function * wrapperFunc) {
    ...
    for (int t = 0; t <= (int)wrapperFunc->arg_size(); ++t) {
        if (t == (int)wrapperFunc->arg_size()) {
            ...
            // LLVM 14: getOrInsertFunction()는 FunctionCallee를 반환, getCallee()로 Constant* 얻기
            FunctionCallee ctorCallee = this->mo->getOrInsertFunction(ctorName, ctorTy);
...
std::vector<Value *> InterTask::genSpawnArgs( CallInst * CI, Function * wrapperFunc) {
    ...
    for (int j = 0; j < (int)CI->arg_size(); j++) {
...
#endif // #if 0 - LLVM 14: InterTask 비활성화