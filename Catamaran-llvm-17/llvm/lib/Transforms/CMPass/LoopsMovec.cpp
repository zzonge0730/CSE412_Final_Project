// LLVM 14: LoopsMovec는 현재 비활성화되어 있으므로, 이 파일도 조건부 컴파일
#if 0
...
// LLVM 14: 새 PassManager를 사용하므로 레거시 RegisterPass 비활성화
...
void LoopsMovec::getAnalysisUsage(AnalysisUsage &AU) const {
    // LLVM 14: 새 PassManager의 Analysis들은 ID가 없어서 레거시 PassManager와 호환되지 않음
    // getAnalysisUsage()를 빈 구현으로 유지 (필요시 나중에 새 PassManager로 전환)
    // TODO: 새 PassManager로 전환 필요
    (void)AU; // unused parameter warning 방지
}
...
std::vector<LoopStructure *> * LoopsMovec::getLoopStructures(void) {
    ...
    for (auto func : *functions) {
        ...
        // LLVM 14: 레거시 PassManager에서 새 PassManager의 Analysis 사용 불가
        // TODO: 새 PassManager로 전환 필요
        // auto& LI = getAnalysis<LoopInfo>(*func);
        // 임시로 비활성화 - LoopInfo 없이 루프를 찾을 수 없으므로 건너뛰기
        continue;
...
std::vector<LoopStructure *> * LoopsMovec::getLoopStructures(Function * func) {
    ...
    // auto& LI = getAnalysis<LoopInfo>(*func);
    // 임시로 비활성화
    // LoopInfo LI; // LoopInfo는 기본 생성자가 없을 수 있음
    // 대신 빈 루프 리스트 반환
    return allLoops;
#if 0
    // LLVM 14: 레거시 PassManager에서 새 PassManager의 Analysis 사용 불가
...
std::vector<Function *> * LoopsMovec::getModuleFunctionsReachableFrom(Module * M, Function * startingPoint) {
    ...
    // TODO: 새 PassManager로 전환 필요
    // 임시로 시작점만 반환
    functions->push_back(startingPoint);
    return functions;
#if 0
...
DominatorSummary * LoopsMovec::getDominators(Function * f) {
    // LLVM 14: 레거시 PassManager의 getAnalysis는 새 PassManager와 호환되지 않음
    // TODO: 새 PassManager로 전환 필요
    // 임시로 직접 DominatorTree와 PostDominatorTree 생성
    ...
    */
    ...
    auto selector = [this, &timeSavedLoops](StayConnectedNestedLoopForestNode *n, uint32_t treeLevel) -> bool {
       ...
       */
        ...
        // LLVM 14: 레거시 PassManager에서 새 PassManager의 Analysis 사용 불가
        // TODO: 새 PassManager로 전환 필요
        // 임시로 비활성화 - LoopInfo와 ScalarEvolution 없이 루프 분석 불가
        return false;
#if 0
...
void LoopsMovec::linkTransformedLoopToOriginalFunction(
    ...
    if (loopExitBlocks.size() == 1) {
        ...
        auto exitEnvPtr = endBuilder.CreateInBoundsGEP(
            envArray->getType()->getPointerElementType(), envArray, indices
        );
        // LLVM 14: CreateLoad는 타입을 명시해야 함
...
bool LoopsMovec::isTheMovecLibraryFunction(Function * libF) {
    ...
    if (movecLibFunction.count(libF->getName().str())) {
        return true;
    }
    return false;
}
...
#endif // #if 0 - LLVM 14: LoopsMovec 비활성화