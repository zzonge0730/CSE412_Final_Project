...
void EnvUserBuilder::createEnvPtr (
  ...
  // LLVM 14: CreateInBoundsGEP API 변경 - SmallVector 사용
  SmallVector<Value*, 2> indices = { zeroV, envIndV };
  // envArray is assumed to be an AllocaInst
  auto allocaInst = cast<AllocaInst>(this->envArray);
  auto envGEP = builder.CreateInBoundsGEP(
    allocaInst->getAllocatedType(),
    this->envArray,
    indices
  );
  auto envPtr = builder.CreateBitCast(envGEP, PointerType::get(builder.getContext(), 0));
...
void EnvUserBuilder::createReducableEnvPtr (
  ...
  auto ptrTy_int8 = PointerType::get(builder.getContext(), 0);
  auto int64 = IntegerType::get(builder.getContext(), 64);
  auto zeroV = cast<Value>(ConstantInt::get(int64, 0));
  auto envIndV = cast<Value>(ConstantInt::get(int64, envIndex * valuesInCacheLine));
  ...
  auto arrPtr = PointerType::get(builder.getContext(), 0);
  auto envReducePtr = builder.CreateBitCast(envReduceGEP, PointerType::get(builder.getContext(), 0));
  ...
  // envReducePtr는 PointerType이므로, element type을 가져옴
  // The loaded value is a pointer to the array of reducers (allocated on stack)
  // We know it is a pointer, but in Opaque Pointers we need to know the type being loaded.
  // It seems we are loading a pointer (to the reducer array) from the environment array slot.
  // The environment array slot holds i64s (or pointers cast to i64/ptr).
  // In `generateEnvVariables`, we stored `reduceArrAlloca` (pointer) into `envPtr`.
  // So we are loading a pointer.
  auto loadedPtrType = PointerType::get(builder.getContext(), 0); 
  auto loadedPtr = builder.CreateLoad(loadedPtrType, envReducePtr);
  
  // We need to GEP into the reducer array. We need the type of the reducer array.
  // The reducer array is ArrayType(int64, ...).
  auto reduceArrType = ArrayType::get(int64, reducerCount * valuesInCacheLine);

  // LLVM 14: CreateInBoundsGEP API 변경 - SmallVector 사용
  SmallVector<Value*, 2> indices2 = { zeroV, reduceIndAlignedV };
  auto envGEP = builder.CreateInBoundsGEP(
    reduceArrType,
    loadedPtr,
    indices2
...
void EnvBuilder::generateEnvVariables (IRBuilder<> builder) {
  ...
  auto fetchCastedEnvPtr = [&](Value *arr, int envIndex, Type *ptrType) -> Value * {
     ...
     */
    // LLVM 14: CreateInBoundsGEP API 변경 - SmallVector 사용
    SmallVector<Value*, 2> indices = { zeroV, indValue };
    auto allocaInst = cast<AllocaInst>(arr);
    auto envPtr = builder.CreateInBoundsGEP(
      allocaInst->getAllocatedType(),
      arr,
      indices
    );
  ...
  for (auto envIndex : singleIndices) {
    auto ptrType = PointerType::get(builder.getContext(), 0);
  ...
  for (auto envIndex : reducableIndices) {
     ...
     */
    auto reduceArrPtrType = PointerType::get(builder.getContext(), 0);
...
BasicBlock * EnvBuilder::reduceLiveOutVariables (
  ...
  for (auto envIndexInitValue : initialValues) {
    ...
    // LLVM 14: CreateInBoundsGEP API 변경 - SmallVector 사용
    SmallVector<Value*, 2> indices = { zeroV, offsetValue };
    auto allocaInst = cast<AllocaInst>(baseAddressOfReducedVar);
     ...
     */
    auto varType = envTypes[envIndex];
    auto ptrType = PointerType::get(loopBodyBuilder.getContext(), 0);
     ...
     */
    // LLVM 14: CreateLoad는 타입을 명시해야 함
    auto envVar = loopBodyBuilder.CreateLoad(varType, effectiveAddressOfReducedVarProperlyCasted);
