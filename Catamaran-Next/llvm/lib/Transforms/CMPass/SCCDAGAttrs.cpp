...
bool SCCDAGAttrs::checkIfReducible (SCC *scc, LoopSummary &LIS) {
  ...
  for (auto dependency : this->sccToLoopCarriedDependencies.at(scc)) {
    ...
    PHINode *consumerPHI = dyn_cast<PHINode>(consumer);

    if (!consumerPHI) {
      /*
       * LLVM 17 Compatibility:
       * The consumer of a loop-carried dependency might not be a PHINode directly (e.g., GEP inst).
       * In this case, we search for the PHINode within the same SCC that resides in the loop header.
       */
      for (auto nodePair : scc->getInternalNodePairs()) {
        if (auto phi = dyn_cast<PHINode>(nodePair.first)) {
           if (phi->getParent() == rootLoopHeader) {
              consumerPHI = phi;
              break; 
           }
        }
      ...
      if (!consumerPHI) {
        // If still no PHI found, we cannot handle this SCC as a reduction variable
