...
MemoryCloningAnalysis::MemoryCloningAnalysis(LoopStructure * loopS, DominatorSummary& ds, PDG * loopPDG) {
    ...
    for (auto allocation : allocations) {
        ...
        if (!sizeInBitsOptional.has_value()) {
            continue;
        }

        //fetch the size of the stack object
        auto sizeInBits = sizeInBitsOptional.value();
  ...
  while (!allocationUses.empty()) {
    ...
    for (auto user : I->users()) {
      ...
      if (auto store = dyn_cast<StoreInst>(user)) {
        ...
        auto isUseTheDestinationOp = (call->arg_size() == 4) && (call->getArgOperand(0) == I);
        auto isUseTheSourceOp = (call->arg_size() == 4) && (call->getArgOperand(1) == I);
