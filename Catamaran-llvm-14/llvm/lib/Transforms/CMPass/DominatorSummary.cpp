...
// LLVM 14: cloneNodes 템플릿 특수화 정의 (생성자에서 사용되기 전에 정의 필요)
// DTAliases::Node용 특수화
template <>
void DomTreeSummary::cloneNodes<DTAliases::Node> (std::set<DTAliases::Node *> &nodesToClone) {

  /*
   * Clone nodes using DomNodeSummary constructors. Track cloned pairs in map
   */
  std::unordered_map<DTAliases::Node *, DomNodeSummary *> nodeMap;
  ...
  for (auto node : nodesToClone) {
    ...
    // LLVM 14: getChildren() 제거됨, iterator 사용 (begin(), end())
    // node는 DTAliases::Node *이므로 iterator 사용 가능
    for (auto childIt = node->begin(); childIt != node->end(); ++childIt) {
      auto child = *childIt;
      // child는 DTAliases::Node *이므로, nodeMap에서 찾을 때도 DTAliases::Node *로 사용
...
// DomNodeSummary용 특수화
template <>
void DomTreeSummary::cloneNodes<DomNodeSummary> (std::set<DomNodeSummary *> &nodesToClone) {

  /*
   * Clone nodes using DomNodeSummary constructors. Track cloned pairs in map
   */
  std::unordered_map<DomNodeSummary *, DomNodeSummary *> nodeMap;
  for (auto node : nodesToClone) {
    auto summary = new DomNodeSummary(*node);
    nodeMap[node] = summary;
    this->nodes.insert(summary);
  ...
  for (auto node : nodesToClone) {
    auto summary = nodeMap[node];
    auto iDom = node->getIDom();
    if (iDom && nodeMap.find(iDom) != nodeMap.end()) {
      summary->iDom = nodeMap[iDom];
    }

    // LLVM 14: DomNodeSummary는 getChildren() 메서드를 사용
    // DomNodeSummary는 iterator가 없으므로 getChildren() 사용
    auto children = node->getChildren();
    for (auto child : children) {
      // child는 DomNodeSummary *이므로, nodeMap에서 찾을 때도 DomNodeSummary *로 사용
      if (nodeMap.find(child) == nodeMap.end()) continue;
      auto childSummary = nodeMap[child];
      childSummary->parent = summary;
      summary->children.push_back(childSummary);
...
std::set<DTAliases::Node *> DomTreeSummary::collectNodesOfTree (TreeType &T) {
  ...
  // LLVM 14: getRoots()가 제거되고 getRoot()만 있음
  // PostDominatorTree의 경우 여러 root가 있을 수 있으므로 getRootNode() 사용
  auto *rootNode = T.getRootNode();
  if (rootNode) {
...
// LLVM 14: 템플릿 특수화는 이미 위에서 정의되었으므로 명시적 인스턴스화 불필요
// template void DomTreeSummary::cloneNodes<DTAliases::Node>(std::set<DTAliases::Node *> &);

// LLVM 14: 템플릿 인스턴스화 명시 (PostDominatorTree를 위한 collectNodesOfTree)
template std::set<DTAliases::Node *> DomTreeSummary::collectNodesOfTree<PostDominatorTree>(PostDominatorTree &);
template std::set<DTAliases::Node *> DomTreeSummary::collectNodesOfTree<DominatorTree>(DominatorTree &);
