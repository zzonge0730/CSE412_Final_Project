...
// 2단계: 전역 생성자 우선순위 조정 - 레거시 PassManager 전역 생성자보다 먼저 실행
// constructor(101)로 높은 우선순위 설정하여 충돌 방지
extern "C" __attribute__((constructor(101))) __attribute__((visibility("default"))) __attribute__((weak))
...
PreservedAnalyses Loops::run(Module &M, ModuleAnalysisManager &AM) {
    ...
    this->program = &M;
    // TODO: PDGAnalysis도 새 PassManager로 전환 필요
    // 임시로 nullptr로 설정 (PDGAnalysis 수정 후 변경)
    this->pdgAnalysis = nullptr; // &AM.getResult<PDGAnalysis>(M);
    ...
    for (auto treeIt = trees.rbegin(); treeIt != trees.rend(); ++treeIt) {
        ...
        for (auto loop : loopsToParallelize) {
            ...
            if (ls->getNestingLevel() == 1) {
                ...
                // LLVM 14: PDGAnalysis가 비활성화되어 있으므로 임시로 nullptr 사용
                // TODO: 새 PassManager로 전환 후 PDGAnalysis 복구 필요
                PDG * loopFuncPDG = nullptr; // this->pdgAnalysis->getFunctionPDG(*loopFunc);
...
PDG * Loops::getFunctionDependenceGraph(Function * func) {
    // LLVM 14: PDGAnalysis가 비활성화되어 있으므로 임시로 nullptr 반환
    // TODO: 새 PassManager로 전환 후 PDGAnalysis 복구 필요
    return nullptr; // this->pdgAnalysis->getFunctionPDG(*func);
}
