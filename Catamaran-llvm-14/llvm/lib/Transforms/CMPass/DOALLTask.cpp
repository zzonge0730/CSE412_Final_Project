            ...
            if (!this->liveInInitValue[liveIn]) {
                ...
                if (AllocaInst * acInst = dyn_cast<AllocaInst>(liveIn)) {
                    ...
                    GetElementPtrInst * gep = GetElementPtrInst::Create(
                        ptr->getType()->getPointerElementType(), ptr, indices, "zyytmp", this->entryBlock);
                    Value * bound_ptr = gep;
                ...
                if (AllocaInst * acInst = dyn_cast<AllocaInst>(liveIn)) {
                    ...
                    LoadInst * load = new LoadInst(arg->getType()->getPointerElementType(), arg, "zyl_", this->entryBlock);
                    ...
                    SmallVector<Value*, 1> indices = { int_bound };
                    GetElementPtrInst * gep = GetElementPtrInst::Create(
                        ptr->getType()->getPointerElementType(), ptr, indices, "ztmp", this->entryBlock);
                    Value * bound_ptr = gep;
    ...
    */
    ...
    newLoopFuncArgIt != newLoopFunc->arg_end(); ++newLoopFuncArgIt, ++wrapperFuncArgIt) {
        ...
        if (tmpType->isPointerTy()) {
            ...
            BitCastInst * bc = new BitCastInst(&*wrapperFuncArgIt, PointerType::getUnqual(tmpType), "", wrapperFuncEntryBB);
            // LLVM 14: LoadInst 생성자 변경 - 타입 명시 필요
            LoadInst * load = new LoadInst(bc->getType()->getPointerElementType(), bc, "", wrapperFuncEntryBB);
